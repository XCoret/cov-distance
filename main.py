...
def video_feed():
    ...
    return Response(gen(Camara(video_path), 0), mimetype='multipart/x-mixed-replace; boundary=frame')


@app.route('/out_feed')
def out_feed():
    video_path = get_blob('TownCentreXVID_short.mp4')
    return Response(gen(Camara(video_path), 1), mimetype='multipart/x-mixed-replace; boundary=frame')
